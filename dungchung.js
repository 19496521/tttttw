...
const apiUrl = 'http://192.168.74.94:8521';
...
//     var account = { email: name, passWordAccount: pass };
  
//     // Gọi API để xác thực tên đăng nhập và mật khẩu của khách hàng
//     fetch('http://192.168.74.94:8521/api/auth/login', {
//         method: 'POST',
...
//         // Lấy thông tin khách hàng từ API bằng email
//         getByEmail(data.name);
//       })
//       .catch(error => {
//         // Xử lý lỗi
//         console.error(error);
...
//   }
  ...
  function getByEmail(email) {
    fetch(`http://192.168.74.94:8521/api/customer/getByEmail/${email}`)
      .then(response => {
        if (!response.ok) {
...
function logIn(form) {
    ...
    var passWordAccount = form.passWordAccount.value;
    var account = { email: email, passWordAccount: passWordAccount };
  
    fetch('http://localhost:8521/api/auth/login', {
      method: 'POST',
      body: JSON.stringify(account),
      headers: {
        'Content-Type': 'application/json',
                ...
                <div id="login">
                    ...
                    <form onsubmit="return logIn(this);">
                        ...
                        <div class="field-wrap">
                            ...
                            <input name="passWordAccount" type="password" required autocomplete="off" />
                        </div> <!-- pass -->
